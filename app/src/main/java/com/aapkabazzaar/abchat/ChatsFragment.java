...
public class ChatsFragment extends Fragment {
    ...
    public void onStart() {
        ...
        firebaseRecyclerAdapter = new FirebaseRecyclerAdapter<Chats, ChatsFragment.ChatsViewHolder>(chatsOptions) {
            ...
            protected void onBindViewHolder(final ChatsFragment.ChatsViewHolder holder, final int position, Chats model) {
                ...
                mMessagesDatabase.child(list_user_id).addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists()) {
                            int count = 0;
                            for (DataSnapshot seenData : dataSnapshot.getChildren()) {
                                final String check = seenData.child("seen").getValue().toString();

                                if (check.equals("false")) {
                                    count = count + 1;
                                }
                            }
                            holder.setMsgCounter(count);
                        }
                    }
    ...
    public static class ChatsViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setMsgCounter(int counter){
           NotificationBadge notificationBadge = mView.findViewById(R.id.badge);
           if(counter>0){
